...
void Odom::update_position(float ForwardTracker_position, float SidewaysTracker_position, float orientation_deg){
  ...
  float half_orientation_delta_rad = orientation_delta_rad * 0.5;
  if (orientation_delta_rad == 0) {
    local_X_position = Forward_delta;
    local_Y_position = Sideways_delta;
  } else {
    float chord_length_factor = 2 * sin(half_orientation_delta_rad);
    local_X_position = chord_length_factor * ((Forward_delta / orientation_delta_rad) + ForwardTracker_center_distance);
    local_Y_position = chord_length_factor * ((Sideways_delta / orientation_delta_rad) + SidewaysTracker_center_distance); 
  }
  ...
  float global_polar_angle = local_polar_angle - prev_orientation_rad - half_orientation_delta_rad;
